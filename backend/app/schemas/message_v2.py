"""
消息相关Pydantic Schemas（完善版 - 与前端对齐）
"""
...
class UnifiedMessage(BaseSchema):
    ...
    # 数据内容
    raw_data: bytes = Field(..., description="原始数据（后端存储为bytes）")
    ...
    routing_rules: List[str] = Field(default_factory=list, description="应用的路由规则ID列表")
...
class UnifiedMessageResponse(BaseSchema):
    """统一消息响应Schema（用于API返回，raw_data转换为base64）"""
    # 基础标识
    message_id: str = Field(..., description="消息唯一标识")
    timestamp: str = Field(..., description="接收时间戳（ISO格式）")
    ...
    # 数据内容
    raw_data: str = Field(..., description="原始数据（base64编码）")
    data_size: int = Field(..., description="数据大小（字节）")
    parsed_data: Optional[Dict[str, Any]] = Field(None, description="解析后的数据")
    frame_schema_id: Optional[str] = Field(None, description="使用的帧格式ID")
